...
import { LeadIn, LowerThird, PlaceTag } from "@/components/ui/Broadcast";
...
export function DetailScreen({
  ...
  const [first, ...moreParts] = item.videos;

  const recording = (video: ContentDetail["videos"][number], index: number) => (
    <figure key={video.youtube_id}>
      ...
      <figcaption className="mt-2 flex items-center justify-between gap-4 text-meta text-ink-500">
        <span>
          {video.label || (item.videos.length > 1 ? `Part ${index + 1}` : "")}
        </span>
  ...
  // Laid out like a watch page: the recording first, the title and the
  // transcript under it, and the entry's details in a panel beside it.
  return (
    <Container className="pb-section-sm pt-5 lg:pt-6">
      ...
      <div className="mt-4 grid gap-8 lg:grid-cols-12 lg:gap-10">
        <div className="lg:col-span-8">
          {first ? recording(first, 0) : null}

          {item.kicker ? <LeadIn className="mt-6">{item.kicker}</LeadIn> : null}
          <h1 className={`text-h3 font-bold text-primary-900 ${item.kicker ? "mt-2" : "mt-5"}`}>
            {item.title}
          </h1>
          {item.speaker ? <p className="mt-1.5 text-meta text-ink-600">{item.speaker}</p> : null}

          {moreParts.length > 0 ? (
            <div className="mt-8 space-y-6">
              {moreParts.map((video, index) => recording(video, index + 1))}
            </div>
          ) : null}

          <div className="mt-6 border-t border-ink-100 pt-6">
            {item.body ? (
              <Prose size="lg" className="whitespace-pre-line">
                {item.body}
              </Prose>
            ) : (
              <p className="max-w-prose text-body-sm text-ink-500">
        ...
        <aside className="lg:col-span-4">
          <div className="rounded-md border border-ink-200 bg-ink-0 p-5 shadow-sm">
            ...
            <dl className="mt-5 space-y-3 border-t border-ink-100 pt-5 text-body-sm">
              <div>
